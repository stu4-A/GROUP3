...
cloudinary.config(
    cloud_name='Reports',
    api_key='838999993676654',
    api_secret='UXIdJVw_bt2lW9kC0g6MkDWTDdk'
)

# -------------------------------
# CSV Download
# -------------------------------
def download_student_grades_csv(request, semester_id=None):
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="student_grades.csv"'

    writer = csv.writer(response)
    writer.writerow(['Student Name', 'Course', 'Grade'])
    # Example: populate with data
    # students = StudentProfile.objects.all()
    # for student in students:
    #     writer.writerow([student.user.get_full_name(), student.course.name, student.grade])
...
def upload_file_to_cloudinary(request):
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        try:
            ...
            messages.success(request, f'File uploaded successfully! URL: {url}')
            return JsonResponse({'url': url})
        except Exception as e:
            messages.error(request, f'Upload failed: {str(e)}')
            return JsonResponse({'error': str(e)}, status=500)
    return render(request, 'careers/upload_file.html')


@login_required
def download_file_from_cloudinary(request, public_id):
    try:
        resource = cloudinary.api.resource(public_id)
        url = resource.get('secure_url')
        return redirect(url)
    except Exception as e:
        messages.error(request, f'File retrieval failed: {str(e)}')
